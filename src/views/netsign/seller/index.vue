...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        djywzmmcbm: [
          { required: true, message: "登记业务证明证书名称编码不能为空", trigger: "blur" }
        ],
        djywzmhm: [
          { required: true, message: "登记业务证明证书号不能为空", trigger: "blur" }
        ],
        jyzlbbm: [
        ...
        jyzxzbm: [
          { required: true, message: "交易者性质编码不能为空", trigger: "blur" }
        ],
        jyzhj: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除买方信息编号为"' + sellerIds + '"的数据项？').then(function() {
